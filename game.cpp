...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (damage.primary.value > 0) {
		...
		if (target->hasCondition(CONDITION_MANASHIELD) && damage.primary.type != COMBAT_UNDEFINEDDAMAGE) {
			...
			// Critical hit: chance determines whether the hit is critical, and critical damage adds a percentage to the final damage.
			if (attackerPlayer && attacker != target && damage.origin != ORIGIN_NONE) {
				const int32_t criticalHitChance = std::min<int32_t>(100, attackerPlayer->getCriticalHitChance());
				if (criticalHitChance > 0 && normal_random(1, 100) <= criticalHitChance) {
					const int32_t criticalDamage = std::max<int32_t>(0, attackerPlayer->getCriticalDamage());
					if (criticalDamage > 0) {
						const int32_t bonusDamage = std::round((damage.primary.value + damage.secondary.value) * (criticalDamage / 100.0));
		...
		if (!applyingReflectDamage && attacker && attacker != target && targetPlayer && realDamage > 0) {
			const int32_t reflectPercent = std::min<int32_t>(100, std::max<int32_t>(0, targetPlayer->getReflect()));
			if (reflectPercent > 0) {
				const int32_t reflectedDamage = static_cast<int32_t>(realDamage * reflectPercent / 100.0);
				if (reflectedDamage > 0) {
					CombatDamage reflectDamage;
					reflectDamage.primary.type = COMBAT_PHYSICALDAMAGE;
					reflectDamage.primary.value = -reflectedDamage;
					reflectDamage.secondary.type = COMBAT_NONE;
					reflectDamage.secondary.value = 0;
					reflectDamage.origin = ORIGIN_NONE;

					applyingReflectDamage = true;
		...
		// Life/Mana Leech: restores the attacker from the damage actually dealt.
		if (!applyingReflectDamage && attacker && attacker != target && realDamage > 0 && attackerPlayer) {
			const int32_t lifeLeechPercent = std::min<int32_t>(100, std::max<int32_t>(0, attackerPlayer->getLifeLeech()));
			const int32_t manaLeechPercent = std::min<int32_t>(100, std::max<int32_t>(0, attackerPlayer->getManaLeech()));

			if (lifeLeechPercent > 0) {
				const int32_t lifeLeechAmount = static_cast<int32_t>(realDamage * lifeLeechPercent / 100.0);
				if (lifeLeechAmount > 0) {
					attacker->gainHealth(nullptr, lifeLeechAmount);
					g_game.addAnimatedText("+" + std::to_string(lifeLeechAmount), attacker->getPosition(), TEXTCOLOR_LIGHTGREEN);
				}
			}
			if (manaLeechPercent > 0) {
				const int32_t manaLeechAmount = static_cast<int32_t>(realDamage * manaLeechPercent / 100.0);
				if (manaLeechAmount > 0) {
					attacker->changeMana(manaLeechAmount);
					g_game.addAnimatedText("+" + std::to_string(manaLeechAmount), attacker->getPosition(), TEXTCOLOR_LIGHTGREY);
				}
			}
		}

		// Life/Mana Absorb: restores the target from the damage actually received.
		if (!applyingReflectDamage && attacker && attacker != target && realDamage > 0 && targetPlayer) {
			const int32_t lifeAbsorbPercent = std::min<int32_t>(100, std::max<int32_t>(0, targetPlayer->getLifeAbsorb()));
			const int32_t manaAbsorbPercent = std::min<int32_t>(100, std::max<int32_t>(0, targetPlayer->getManaAbsorb()));

			if (lifeAbsorbPercent > 0) {
				const int32_t lifeAbsorbAmount = static_cast<int32_t>(realDamage * lifeAbsorbPercent / 100.0);
				if (lifeAbsorbAmount > 0) {
					target->gainHealth(nullptr, lifeAbsorbAmount);
					g_game.addAnimatedText("+" + std::to_string(lifeAbsorbAmount), target->getPosition(), TEXTCOLOR_LIGHTGREEN);
				}
			}
			if (manaAbsorbPercent > 0) {
				const int32_t manaAbsorbAmount = static_cast<int32_t>(realDamage * manaAbsorbPercent / 100.0);
				if (manaAbsorbAmount > 0) {
					target->changeMana(manaAbsorbAmount);
					g_game.addAnimatedText("+" + std::to_string(manaAbsorbAmount), target->getPosition(), TEXTCOLOR_LIGHTGREY);
				}
			}
		}
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, int32_t manaChange, CombatOrigin origin)
{
	if (manaChange > 0) {
		...
		// Leech/Absorb also work from mana damage.
		if (attacker && attacker != target && manaLoss > 0 && attackerPlayer) {
			const int32_t lifeLeechPercent = std::min<int32_t>(100, std::max<int32_t>(0, attackerPlayer->getLifeLeech()));
			const int32_t manaLeechPercent = std::min<int32_t>(100, std::max<int32_t>(0, attackerPlayer->getManaLeech()));

			if (lifeLeechPercent > 0) {
				const int32_t lifeLeechAmount = static_cast<int32_t>(manaLoss * lifeLeechPercent / 100.0);
				if (lifeLeechAmount > 0) {
					attacker->gainHealth(nullptr, lifeLeechAmount);
					g_game.addAnimatedText("+" + std::to_string(lifeLeechAmount), attacker->getPosition(), TEXTCOLOR_LIGHTGREEN);
				}
			...
			if (manaLeechPercent > 0) {
				const int32_t manaLeechAmount = static_cast<int32_t>(manaLoss * manaLeechPercent / 100.0);
				if (manaLeechAmount > 0) {
					attacker->changeMana(manaLeechAmount);
					g_game.addAnimatedText("+" + std::to_string(manaLeechAmount), attacker->getPosition(), TEXTCOLOR_BLUE);
				}
			}
		}

		if (attacker && attacker != target && manaLoss > 0 && targetPlayer) {
			const int32_t lifeAbsorbPercent = std::min<int32_t>(100, std::max<int32_t>(0, targetPlayer->getLifeAbsorb()));
			const int32_t manaAbsorbPercent = std::min<int32_t>(100, std::max<int32_t>(0, targetPlayer->getManaAbsorb()));

			if (lifeAbsorbPercent > 0) {
				const int32_t lifeAbsorbAmount = static_cast<int32_t>(manaLoss * lifeAbsorbPercent / 100.0);
				if (lifeAbsorbAmount > 0) {
					target->gainHealth(nullptr, lifeAbsorbAmount);
					g_game.addAnimatedText("+" + std::to_string(lifeAbsorbAmount), target->getPosition(), TEXTCOLOR_LIGHTGREEN);
				}
			...
			if (manaAbsorbPercent > 0) {
				const int32_t manaAbsorbAmount = static_cast<int32_t>(manaLoss * manaAbsorbPercent / 100.0);
				if (manaAbsorbAmount > 0) {
					target->changeMana(manaAbsorbAmount);
					g_game.addAnimatedText("+" + std::to_string(manaAbsorbAmount), target->getPosition(), TEXTCOLOR_BLUE);
				}
			}
		}
