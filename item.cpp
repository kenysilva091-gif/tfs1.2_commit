...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch (attr) {
		...
		case ATTR_CRITICALHITCHANCE: {
			...
			setIntAttr(ITEM_ATTRIBUTE_CRITICALHITCHANCE, value);
			break;
		}
		...
		case ATTR_DODGECHANCE: {
			...
			setIntAttr(ITEM_ATTRIBUTE_DODGECHANCE, value);
			break;
		}
		...
		case ATTR_LIFEABSORB: {
			...
			setIntAttr(ITEM_ATTRIBUTE_LIFEABSORB, value);
			break;
		}
		...
		case ATTR_MANAABSORB: {
			...
			setIntAttr(ITEM_ATTRIBUTE_MANAABSORB, value);
			break;
		}
		...
		case ATTR_LIFELEECH: {
			...
			setIntAttr(ITEM_ATTRIBUTE_LIFELEECH, value);
			break;
		}
		...
		case ATTR_MANALEECH: {
			...
			setIntAttr(ITEM_ATTRIBUTE_MANALEECH, value);
			break;
		}
		...
		case ATTR_REFLECT: {
			...
			setIntAttr(ITEM_ATTRIBUTE_REFLECT, value);
			break;
		}
		...
		case ATTR_MOMENTUM: {
			...
			setIntAttr(ITEM_ATTRIBUTE_MOMENTUM, value);
			break;
		}
...
void Item::serializeAttr(PropWriteStream& propWriteStream) const
{
	...
	if (hasAttribute(ITEM_ATTRIBUTE_CRITICALHITCHANCE)) {
		...
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_CRITICALHITCHANCE));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_DODGECHANCE)) {
		propWriteStream.write<uint8_t>(ATTR_DODGECHANCE);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_DODGECHANCE));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_LIFEABSORB)) {
		propWriteStream.write<uint8_t>(ATTR_LIFEABSORB);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_LIFEABSORB));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_MANAABSORB)) {
		propWriteStream.write<uint8_t>(ATTR_MANAABSORB);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_MANAABSORB));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_LIFELEECH)) {
		propWriteStream.write<uint8_t>(ATTR_LIFELEECH);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_LIFELEECH));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_MANALEECH)) {
		propWriteStream.write<uint8_t>(ATTR_MANALEECH);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_MANALEECH));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_REFLECT)) {
		propWriteStream.write<uint8_t>(ATTR_REFLECT);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_REFLECT));
	}
                                 ...
                                 const Item* item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/)
{
	...
	if (it.isRune()) {
		...
		if (it.weaponType == WEAPON_DISTANCE && it.ammoType != AMMO_NONE) {
			...
			const int32_t criticalHitChance = item ? item->getCriticalHitChance() : it.criticalHitChance;
			const int32_t dodgeChance = item ? item->getDodgeChance() : it.dodgeChance;
			const int32_t lifeAbsorb = item ? item->getLifeAbsorb() : it.lifeAbsorb;
			const int32_t manaAbsorb = item ? item->getManaAbsorb() : it.manaAbsorb;
			const int32_t lifeLeech = item ? item->getLifeLeech() : it.lifeLeech;
			const int32_t manaLeech = item ? item->getManaLeech() : it.manaLeech;
			const int32_t reflect = item ? item->getReflect() : it.reflect;
			const int32_t momentum = item ? item->getMomentum() : it.momentum;
			const int32_t criticalDamage = item ? item->getCriticalDamage() : it.criticalDamage;
			if (criticalHitChance) s << ", Crit Chance:" << std::showpos << criticalHitChance << '%' << std::noshowpos;
			if (dodgeChance) s << ", Dodge Chance:" << std::showpos << dodgeChance << '%' << std::noshowpos;
			if (lifeAbsorb) s << ", Life Absorb:" << std::showpos << lifeAbsorb << '%' << std::noshowpos;
			if (manaAbsorb) s << ", Mana Absorb:" << std::showpos << manaAbsorb << '%' << std::noshowpos;
			if (lifeLeech) s << ", Life Leech:" << std::showpos << lifeLeech << '%' << std::noshowpos;
			if (manaLeech) s << ", Mana Leech:" << std::showpos << manaLeech << '%' << std::noshowpos;
			if (reflect) s << ", Reflect:" << std::showpos << reflect << '%' << std::noshowpos;
			if (momentum) s << ", Momentum:" << std::showpos << momentum << '%' << std::noshowpos;
			if (criticalDamage) s << ", Crit Damage:" << std::showpos << criticalDamage << '%' << std::noshowpos;
			...
			const auto appendPercentAttribute = [&](const char* name, int32_t value) {
				if (value == 0) {
					return;
				}
				if (begin) {
				...
				s << name << ':' << std::showpos << value << '%' << std::noshowpos;
			};

			appendPercentAttribute("Critical Chance", criticalHitChance);
			appendPercentAttribute("Dodge Chance", dodgeChance);
			appendPercentAttribute("Life Absorb", lifeAbsorb);
			appendPercentAttribute("Mana Absorb", manaAbsorb);
			appendPercentAttribute("Life Leech", lifeLeech);
			appendPercentAttribute("Mana Leech", manaLeech);
			appendPercentAttribute("Reflect", reflect);
			appendPercentAttribute("Momentum", momentum);
			appendPercentAttribute("Crit Damage", criticalDamage);
		...
		it.criticalHitChance != 0 || it.criticalDamage != 0 || it.dodgeChance != 0 || it.lifeAbsorb != 0 || it.manaAbsorb != 0 ||
		it.lifeLeech != 0 || it.manaLeech != 0 || it.reflect != 0 || it.momentum != 0 ||
		(item && (item->getCriticalHitChance() != 0 || item->getDodgeChance() != 0 || item->getLifeAbsorb() != 0 ||
			(item && (item->getCriticalHitChance() != 0 || item->getCriticalDamage() != 0 || item->getDodgeChance() != 0 || item->getLifeAbsorb() != 0 ||
		bool begin = true;
		...
		appendPercentAttribute("Crit Chance", criticalHitChance);
		appendPercentAttribute("Dodge Chance", dodgeChance);
		appendPercentAttribute("Life Absorb", lifeAbsorb);
		appendPercentAttribute("Mana Absorb", manaAbsorb);
		appendPercentAttribute("Life Leech", lifeLeech);
