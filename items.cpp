...
void Items::parseItemNode(const pugi::xml_node& itemNode, uint16_t id)
{
	...
	for (auto attributeNode : itemNode.children()) {
		...
		if (tmpStrValue == "type") {
			...
			it.lifeAbsorb = pugi::cast<int32_t>(valueAttribute.value());
		} else if (tmpStrValue == "manaabsorb") {
			it.manaAbsorb = pugi::cast<int32_t>(valueAttribute.value());
		} else if (tmpStrValue == "lifeleech") {
			it.lifeLeech = pugi::cast<int32_t>(valueAttribute.value());
		} else if (tmpStrValue == "manaleech") {
			it.manaLeech = pugi::cast<int32_t>(valueAttribute.value());
		} else if (tmpStrValue == "reflect") {
			it.reflect = pugi::cast<int32_t>(valueAttribute.value());
		} else if (tmpStrValue == "momentum") {
			it.momentum = pugi::cast<int32_t>(valueAttribute.value());
