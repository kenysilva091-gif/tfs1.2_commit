...
{
	int32_t value = 0;
	int32_t relicValue = 0;
	getStorageValue(32500, relicValue);
	value += relicValue;
	for (Item* item : inventory) {
		if (item) value += item->getCriticalHitChance();
	}
	return value;
}

int32_t Player::getCriticalDamage() const
{
	int32_t value = 0;
	for (Item* item : inventory) {
		if (item) value += item->getCriticalDamage();
	}
	return value;
}
...
int32_t Player::getDodgeChance() const
{
	int32_t value = 0;
	int32_t relicValue = 0;
	getStorageValue(32501, relicValue);
	value += relicValue;
	for (Item* item : inventory) {
		if (item) value += item->getDodgeChance();
	}
	return value;
}
...
int32_t Player::getLifeAbsorb() const
{
	int32_t value = 0;
	int32_t relicValue = 0;
	getStorageValue(32502, relicValue);
	value += relicValue;
	for (Item* item : inventory) {
		if (item) value += item->getLifeAbsorb();
	}
	return value;
}
...
int32_t Player::getManaAbsorb() const
{
	int32_t value = 0;
	int32_t relicValue = 0;
	getStorageValue(32503, relicValue);
	value += relicValue;
	for (Item* item : inventory) {
		if (item) value += item->getManaAbsorb();
	}
	return value;
}
...
int32_t Player::getLifeLeech() const
{
	int32_t value = 0;
	int32_t relicValue = 0;
	getStorageValue(32504, relicValue);
	value += relicValue;
	for (Item* item : inventory) {
		if (item) value += item->getLifeLeech();
	}
	return value;
}
...
int32_t Player::getManaLeech() const
{
	int32_t value = 0;
	int32_t relicValue = 0;
	getStorageValue(32505, relicValue);
	value += relicValue;
	for (Item* item : inventory) {
		if (item) value += item->getManaLeech();
	}
	return value;
}

int32_t Player::getReflect() const
{
	int32_t relicValue = 0;
	getStorageValue(32506, relicValue);
	...
	return shield->getReflect() + relicValue;
}

int32_t Player::getMomentum() const
{
	int32_t value = 0;

	// Momentum is activated exclusively by equipped items with the momentum attribute.
	for (Item* item : inventory) {
		if (item) {
			value += item->getMomentum();
		}
	}
...
void Player::onThink(uint32_t interval)
{
	...
	// Momentum: every 2 seconds, roll a chance to reduce active spell exhaustion by 2 seconds.
	momentumTicks += interval;
	if (momentumTicks >= 2000) {
		momentumTicks -= 2000;

		const int32_t momentum = std::min<int32_t>(100, std::max<int32_t>(0, getMomentum()));
		if (momentum > 0 && uniform_random(1, 100) <= momentum) {
			bool reduced = false;

			if (Condition* combatExhaust = getCondition(CONDITION_EXHAUST_COMBAT, CONDITIONID_COMBAT, 1)) {
				const int32_t ticks = combatExhaust->getTicks();
				if (ticks > 0) {
					combatExhaust->setTicks(std::max<int32_t>(0, ticks - 2000));
					reduced = true;
				}
			}

			if (Condition* healExhaust = getCondition(CONDITION_EXHAUST_HEAL, CONDITIONID_COMBAT, 1)) {
				const int32_t ticks = healExhaust->getTicks();
				if (ticks > 0) {
					healExhaust->setTicks(std::max<int32_t>(0, ticks - 2000));
					reduced = true;
				}
			}

			if (reduced) {
				g_game.addAnimatedText("MOMENTUM!", getPosition(), TEXTCOLOR_YELLOW);
				g_game.addMagicEffect(getPosition(), 40);
			}
		}
	}
...
void Player::addCombatExhaust(uint32_t ticks, uint32_t subId)
{
	Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_EXHAUST_COMBAT, ticks, 0, false, subId);
	addCondition(condition);
}

void Player::addHealExhaust(uint32_t ticks, uint32_t subId)
{
	Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_EXHAUST_HEAL, ticks, 0, false, subId);
	addCondition(condition);
}
...
Item* Player::getPouchBag() const
{
	Item* backpack = inventory[CONST_SLOT_BACKPACK];
	if (!backpack) {
		return nullptr;
	}

	Container* mainBackpack = backpack->getContainer();
	if (!mainBackpack) {
		return nullptr;
	}
	...
	for (ContainerIterator it = mainBackpack->iterator(); it.hasNext(); it.advance()) {
		Item* item = *it;
		if (item && item->getID() == ITEM_POUCH_BAG) {
			return item;
		}
	}
