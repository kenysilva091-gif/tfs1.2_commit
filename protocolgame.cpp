...
void ProtocolGame::login(const std::string& name, uint32_t accountId, OperatingSystem_t operatingSystem)
{
	...
	if (!_player || g_config.getBoolean(ConfigManager::ALLOW_CLONES)) {
		...
		if (operatingSystem >= CLIENTOS_OTCLIENT_LINUX) {
			...
			player->registerCreatureEvent("CastExtendedOpcode");
		}
...
void ProtocolGame::parsePacket(NetworkMessage& msg)
{
	...
	// Cast spectators are read-only. They may chat, use ExtendedOpcode
	// (including !cast off), and logout, but cannot perform gameplay actions.
	int32_t castSpectatorStorage = -1;
	const bool isCastSpectator = player->getStorageValue(32441, castSpectatorStorage) && castSpectatorStorage > 0;
	if (isCastSpectator && recvbyte != 0x14 && recvbyte != 0x32 && recvbyte != 0x96) {
		return;
	}
